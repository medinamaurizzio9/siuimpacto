...
{
    public function receipt(Request $request, CashMovement $cashMovement)
    {
        ...
        return Pdf::loadView('pdf.recibo', ['movimiento' => $cashMovement->load('cliente', 'venta', 'cuota')])
            ->stream('recibo-'.$cashMovement->id.'.pdf');
    }

    public function paymentPlan(Request $request, Venta $venta)
    {
        ...
        return Pdf::loadView('pdf.plan-pagos', ['venta' => $venta->load('cliente', 'lote.manzano.urbanizacion', 'cuotas')])
            ->stream('plan-pagos-'.$venta->id.'.pdf');
    }

    public function contract(Request $request, Venta $venta)
    {
        $this->authorizeClientOrPermission($request, $venta->cliente_id, 'ver ventas');
        if (! $request->user()->hasRole('cliente')) {
            abort_unless(UrbanizacionContext::ventaBelongsToCurrent($venta), 403, 'No tienes acceso a esta urbanizacion');
        ...
        return Pdf::loadView('pdf.contrato', ['venta' => $venta->load('cliente', 'lote.manzano.urbanizacion', 'cuotas')])
